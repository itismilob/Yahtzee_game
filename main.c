...
void lock() {
    ...
    while (1) {
        ...
        if (numbers[0] == 5 || numbers[1] == 5 || numbers[2] == 5 || numbers[3] == 5 || numbers[4] == 5 ||
            numbers[5] == 5) {
            // 첫 번째 얏찌 확인
            printf("\nYahtzee!\n");
        }
        ...
        if (remain > 0 && isStop == 0) {
            ...
            getInput();
            if (!strcmp(user_input, "1") || !strcmp(user_input, "2") || !strcmp(user_input, "3") ||
                !strcmp(user_input, "4") || !
                        strcmp(user_input, "5")) {
                // lock을 변경함 1->0 0->1
                int temp = atoi(user_input) - 1;
                locking[temp] = locking[temp] == 1 ? 0 : 1;
            } else if (strcmp(user_input, "roll") == 0) {
                roll();
            ...
            // 조합 선택이 완료된건지 back을 선택한건지 확인함
            if (combi() == 1) {
                // 조합 선이 끝나고 턴이 끝나면 다음 유저로 넘어감
...
int combi() {
    while (1) {
        ...
        for (i = 0; i < 13; i++) {
            if (strcmp(user_input, selectedList[i]) == 0) {
                selected = i;
                break;
            }
        }
